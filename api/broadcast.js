...
const BROADCAST_FILE = path.join(__dirname, '..', '.broadcast_announcement.json');
        ...
        if (fs.existsSync(BROADCAST_FILE)) {
            ...
            if (parsed && typeof parsed === 'object') {
                return {
                    ...
                    badge: typeof parsed.badge === 'string' ? parsed.badge : 'PENGUMUMAN',
                    type: parsed.type || 'info',
                    speed: parsed.speed || 'normal',
                    icon: parsed.icon || 'megaphone',
                    displayMode: parsed.displayMode || 'both',
                    closable: parsed.closable !== false,
...
function saveBroadcast(config) {
    try {
        fs.writeFileSync(BROADCAST_FILE, JSON.stringify(config, null, 2), 'utf8');
        return true;
    } catch (e) {
        return false;
    }
