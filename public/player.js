...
function handleImgError(img) {
    if (!img) return;
    var isArtist = img.classList.contains('artist-photo') || 
                   img.classList.contains('rounded-full') || 
                   (img.closest && (img.closest('#home-section-artis-populer') || img.closest('#artist-modal') || img.closest('#artist-content')));
    var fallback = isArtist ? FA : FI;
...
function parseDurationSec(dur) {
    ...
    var str = String(dur).trim().replace(/\./g, ':');
    if (str.includes(':')) {
        var parts = str.split(':');
        if (parts.length === 3) {
...
function updateOG(title,image){
    var t=document.querySelector('meta[property="og:title"]');if(!t){t=document.createElement('meta');t.setAttribute('property','og:title');document.head.appendChild(t);}t.setAttribute('content',title+' | MusifyStar');
    ...
    document.title=title+' - MusifyStar';
}
...
function getAuthoritativeDuration() {
    var knownDur = (S.ct && S.ct.duration) ? parseDurationSec(S.ct.duration) : 0;
    if (knownDur > 0) {
        if (AU.duration && !isNaN(AU.duration) && isFinite(AU.duration) && AU.duration > 0) {
            if (Math.abs(AU.duration - knownDur) <= Math.max(15, knownDur * 0.15)) {
                return AU.duration;
            }
        }
        return knownDur;
...
AU.addEventListener('loadedmetadata', function(){
    var d = getAuthoritativeDuration();
    if(d > 0){
        S.pd = d;
        renderProgress();
    }
...
AU.addEventListener('play',function(){S.ip=true;S.il=false;UB();SP();try{AU.playbackRate=S.playbackRate||1.0;}catch(ex){}sendPlaybackHeartbeat(true);requestAudioWakeLock();});
AU.addEventListener('pause',function(){if(!AU.ended && !S.il){S.ip=false;UB();ST();sendPlaybackHeartbeat(false);releaseAudioWakeLock();}});
AU.addEventListener('waiting',function(){S.il=true;UB();});
AU.addEventListener('playing',function(){S.ip=true;S.il=false;UB();sendPlaybackHeartbeat(true);});
...
function sendPlaybackHeartbeat(isPlaying) {
    var now = Date.now();
    if (isPlaying && (now - lastHeartbeatTime < 4000)) return;
...
// Screen Keep-Alive & Background Auto-Resume Sync
...
document.addEventListener('visibilitychange', function() {
    if (!document.hidden) {
        if (typeof S !== 'undefined' && S.ip) {
            requestAudioWakeLock();
            if (AU && AU.paused && AU.src) {
...
function updateVolumeUI() {
    ...
    if (volProgress) {
        ...
        if (pct === 0) {
            volProgress.style.boxShadow = 'none';
        } else {
            volProgress.style.boxShadow = '0 0 10px rgba(255,255,255,0.4)';
        }
    }
    ...
    var volBtn = gid('full-vol-icon-btn');
    if (volIcon) {
        if (pct === 0 && volBtn) {
            volBtn.classList.add('text-rose-400', 'bg-rose-500/20', 'border-rose-500/30');
            volBtn.classList.remove('text-white/80', 'bg-white/[0.06]', 'border-white/10');
        } else if (volBtn) {
            volBtn.classList.remove('text-rose-400', 'bg-rose-500/20', 'border-rose-500/30');
            volBtn.classList.add('text-white/80', 'bg-white/[0.06]', 'border-white/10');
        }
        volIcon.setAttribute('data-lucide', iconName);
    }

    var headerVolIcon = gid('full-vol-header-icon');
    if (headerVolIcon) {
        headerVolIcon.setAttribute('data-lucide', iconName);
    }
...
function renderProgress(){
    var p=(S.pd>0 && S.pt>0)?(S.pt/S.pd)*100:0;
    ...
    if(sb){
        sb.value=p;
        sb.style.pointerEvents = S.il ? 'none' : 'auto';
    }
    if(tc)tc.innerText=fm(S.pt);
    if(td)td.innerText=(S.pd>0 ? fm(S.pd) : (S.il ? '--:--' : '0:00'));
    ULH(S.pt);

    var mcp = gid('mini-circle-progress');
    if (mcp) {
        var totalLen = 131.95;
        var offset = S.il ? totalLen : Math.max(0, totalLen * (1 - (p / 100)));
        mcp.style.strokeDashoffset = offset;
    }
...
function updateAutoNextTransition(progress, nextTrack) {
    ...
    // 3. Full Player Background Blur Artwork & App Background Blur
    ...
    var appBgNext = gid('app-bg-artwork-next');
    if (appBgNext) {
        appBgNext.style.display = 'block';
        if (appBgNext.getAttribute('data-vid') !== nextVid) {
            appBgNext.src = nextCover;
            appBgNext.onerror = function(){ handleImgError(this); };
            appBgNext.setAttribute('data-vid', nextVid);
        }
        appBgNext.style.opacity = opacityRatio;
    }
...
function resetAutoNextTransition() {
    ...
    var appBgNext = gid('app-bg-artwork-next');
    if (appBgNext) {
        appBgNext.style.opacity = '0';
        appBgNext.style.display = 'none';
    }
...
function UB(){
    ...
    if(S.il){
        ...
        fu.innerHTML='<div class="w-7 h-7 border-3 border-current border-t-transparent rounded-full animate-spin"></div>';
        ...
        if(fullCover){
            if(S.ct){
                fullCover.style.transform='scale(0.98)';
                fullCover.style.filter='brightness(0.95)';
            }else{
                fullCover.style.transform='scale(1)';
                fullCover.style.filter='brightness(1)';
            }
...
function updateCoverWithTransition(imgEl, newSrc, origCover, useScale) {
    ...
    tempImg.onload = function() {
        ...
        // Detect YouTube gray placeholder (120x90) and fallback
        if (tempImg.naturalWidth <= 120 && tempImg.naturalHeight <= 90) {
            isResolved = true;
        ...
        imgEl.style.opacity = '1';
        if (useScale) imgEl.style.transform = 'scale(1)';
        if (imgEl.id === 'full-cover' && typeof FullPlayer !== 'undefined' && FullPlayer.onCoverLoaded) {
            FullPlayer.onCoverLoaded(tempImg.naturalWidth, tempImg.naturalHeight);
        }
    };
    tempImg.onerror = function() {
        ...
        if (imgEl.id === 'full-cover' && typeof FullPlayer !== 'undefined' && FullPlayer.onCoverLoaded) {
            setTimeout(function() {
                if (imgEl.naturalWidth) FullPlayer.onCoverLoaded(imgEl.naturalWidth, imgEl.naturalHeight);
            }, 100);
        }
    };
    tempImg.src = target;
...
function updateOG(title, cover, artist) {
    if (title && cover) {
        ...
        var docTitle = fullTitle + ' | MusifyStar';
        var description = 'Dengarkan ' + fullTitle + ' di MusifyStar';
        ...
        document.title = 'MusifyStar';

        setMetaTag('og:title', 'MusifyStar', true);
        setMetaTag('og:description', 'MusifyStar - Web Music Player', true);
        setMetaTag('og:image', defaultCover, true);
        setMetaTag('og:image:width', '600', true);
        setMetaTag('og:image:height', '600', true);
        setMetaTag('og:url', location.href, true);
        setMetaTag('twitter:card', 'summary_large_image', false);
        setMetaTag('twitter:title', 'MusifyStar', false);
        setMetaTag('twitter:description', 'MusifyStar - Web Music Player', false);
...
function updateOGForArtist(artistName, coverUrl) {
    ...
    var docTitle = name + ' - Artist | MusifyStar';
    var description = 'Dengarkan lagu dan album terbaik dari ' + name + ' di MusifyStar';
...
function updateOGForAlbum(albumTitle, coverUrl, artistName) {
    ...
    var docTitle = fullTitle + ' - Album | MusifyStar';
    var description = 'Dengarkan album ' + fullTitle + ' di MusifyStar';
    var cover = (coverUrl && coverUrl !== FI) ? coverUrl : 'https://www.gobox.my.id/file/R0ym4wqfznmp.png';
...
function UU(){
    ...
    if (typeof FullPlayer !== 'undefined' && FullPlayer.resetCoverState) {
        FullPlayer.resetCoverState();
    }

    var mc=gid('mini-cover'),mt=gid('mini-title'),ma=gid('mini-artist'),fc=gid('full-cover'),ft=gid('full-title'),fa=gid('full-artist'),fh=gid('full-header-artist'),fb=gid('full-bg-blur'),fba=gid('full-bg-artwork'),ab=gid('app-bg-artwork');
    ...
    if(ab) updateCoverWithTransition(ab, hdCover, origCover, false);
    ...
    if(typeof FullPlayer !== 'undefined' && FullPlayer.applyCoverMode) FullPlayer.applyCoverMode();
}
...
function PK(s,i){
    ...
    else if(typeof Home !== 'undefined' && Home.sections && Home.sections[s]) l = Home.sections[s];
    else if(typeof S !== 'undefined' && S.homeSections && S.homeSections[s]) l = S.homeSections[s];
    ...
    if(S.ct && ((S.ct.id && S.ct.id === l[i].id) || (S.ct.videoId && S.ct.videoId === l[i].videoId) || (S.ct.title === l[i].title && S.ct.artist === l[i].artist))){
        TP();
        return;
    }
...
function loadTrack(track,resumeAt){
    ...
    try{AU.removeAttribute('src');AU.src='';AU.load();}catch(e){}
    S.il = true;
    S.ip = false;

    // Immediately reset track progress so it doesn't show previous song's time/duration
    S.pt = resumeAt || 0;
    S.pd = (track && track.duration) ? (parseDurationSec(track.duration) || 0) : 0;
    renderProgress();
    UB();
...
var currentAudioAbortController = null;

async function fetchAudioAndPlay(track,resumeAt){
    if (currentAudioAbortController) {
        try { currentAudioAbortController.abort(); } catch(e){}
    }
    currentAudioAbortController = new AbortController();
    var signal = currentAudioAbortController.signal;
    ...
    try{
        ...
        if (!audioUrl) {
            ...
            if(d&&d.status&&d.result){
                if(d.result.download && d.result.download.audio){
                ...
                if(d.result.duration && (!track.duration || track.duration === '0:00' || track.duration === 'Video')){
                    track.duration = d.result.duration;
                    var sec = parseDurationSec(d.result.duration);
                    if(sec > 0){
                        S.pd = sec;
                        renderProgress();
                    }
        ...
        if(signal.aborted || S.ct!==track)return;
        if(audioUrl){
            ...
            if(p !== undefined && p.then){
                p.then(function(){
                    ...
                    console.warn('[AUDIO] play error or waiting gesture:', err);
                    if (S.ct === track) {
                        S.il = false;
                        S.ip = false;
                        UB();
        ...
        if (e.name !== 'AbortError' && S.ct===track){
            S.il=false;S.ip=false;UB();
        }
    }
}
...
function TP(){
    ...
    if(!AU.src || AU.src === '' || AU.src === window.location.href){
        loadTrack(S.ct);
        return;
    }
    ...
    if(AU.paused){
        ...
        if(p !== undefined && p.then){
            p.then(function(){
                ...
                console.warn('[AUDIO] TP play error, reloading track:', err);
                loadTrack(S.ct);
            });
        }
    } else {
...
function showPlaylistPicker(track){
    ...
    var prev = gid('playlist-picker-modal');
    if (prev) prev.remove();

    var pls = typeof getUserPlaylists === 'function' ? getUserPlaylists() : [];
    var trackTitle = track.title || 'Lagu';
    var trackArtist = track.artist || 'Artis';
    var trackCover = toHDCover(track.cover || track.thumbnail || (typeof FI !== 'undefined' ? FI : '/icon.png'), track.videoId || track.id);

    var popup = document.createElement('div');
    popup.id = 'playlist-picker-modal';
    popup.className = 'fixed inset-0 z-[300] flex items-end justify-center bg-black/70 backdrop-blur-sm select-none';
    ...
    if (pls.length === 0) {
        bodyContentHtml = `
        <div class="rounded-2xl bg-white/[0.04] border border-white/10 p-5 text-center flex flex-col items-center justify-center my-1">
            <div class="w-11 h-11 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/40 mb-2">
                <i data-lucide="music" class="w-5 h-5"></i>
            </div>
            <h4 class="font-bold text-xs sm:text-sm text-white mb-1">Belum Ada Playlist</h4>
            <p class="text-[11px] text-white/50 max-w-xs mb-3 leading-relaxed">Klik tombol "Buat Playlist Baru" di atas untuk membuat koleksi pertamamu.</p>
            <button onclick="popup_createPlaylistForSong();" class="px-3.5 py-1.5 rounded-full bg-white/15 hover:bg-white/25 border border-white/20 text-white text-xs font-bold active:scale-95 transition-all cursor-pointer inline-flex items-center gap-1.5 shadow-sm">
                <i data-lucide="plus" class="w-3.5 h-3.5"></i>
                <span>Buat Sekarang</span>
            </button>
        </div>`;
    } else {
        var itemsHtml = pls.map(function(p) {
            var pCover = p.image || (p.songs && p.songs.length > 0 ? p.songs[0].cover : trackCover);
            var isAdded = p.songs && p.songs.some(function(s) { 
                return (track.videoId && s.videoId === track.videoId) || 
                       (track.id && s.id === track.id) || 
                       (s.title === track.title && s.artist === track.artist); 
            });

            return `
            <div onclick="popup_addSongToPlaylist('${p.id}');" class="rounded-2xl bg-white/[0.06] hover:bg-white/[0.12] border border-white/15 hover:border-white/30 p-2 sm:p-2.5 flex items-center gap-2.5 sm:gap-3 cursor-pointer transition-all active:scale-[0.98] group">
                <div class="w-10 h-10 rounded-xl overflow-hidden shrink-0 border border-white/10 bg-black/40 relative">
                    <img src="${pCover}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='${trackCover}'" />
                </div>
                <div class="flex-1 min-w-0">
                    <h4 class="font-bold text-xs sm:text-sm text-white truncate group-hover:text-white/95">${es(p.name)}</h4>
                    <p class="text-[11px] text-white/50 truncate mt-0.5">${(p.songs ? p.songs.length : 0)} lagu</p>
                </div>
                <div class="shrink-0">
                    ${isAdded 
                        ? '<span class="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px] font-bold inline-flex items-center gap-1"><i data-lucide="check" class="w-3 h-3 stroke-[3]"></i> Tersimpan</span>'
                        : '<div class="w-7 h-7 rounded-full bg-white/10 group-hover:bg-white/20 border border-white/20 text-white flex items-center justify-center transition-all"><i data-lucide="plus" class="w-3.5 h-3.5"></i></div>'
                    }
                </div>
            </div>`;
        }).join('');
        ...
        bodyContentHtml = `
        <div class="space-y-2 max-h-56 overflow-y-auto hide-scrollbar my-1 pr-0.5">
            ${itemsHtml}
        </div>`;
    }

    popup.innerHTML = `
    <div class="w-full max-w-md rounded-t-3xl p-4 sm:p-5 border-t border-white/20 shadow-2xl bg-[#10121a]/95 backdrop-blur-2xl" style="animation: slideUp 0.25s cubic-bezier(0.16, 1, 0.3, 1) forwards;">
        <!-- Top Drag Bar -->
        <div class="w-10 h-1 bg-white/25 rounded-full mx-auto mb-3"></div>

        <!-- Header -->
        <div class="flex items-center justify-between mb-3 pb-2.5 border-b border-white/10">
            <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white border border-white/15">
                    <i data-lucide="list-plus" class="w-4 h-4"></i>
                </div>
                <div>
                    <h3 class="font-bold text-sm text-white leading-tight">Tambahkan ke Playlist</h3>
                    <p class="text-[10px] text-white/50 leading-tight mt-0.5">Simpan ke koleksi musik Anda</p>
                </div>
            </div>
            <button onclick="this.closest('.fixed').remove();" class="w-7 h-7 rounded-full bg-white/10 hover:bg-white/20 text-white/70 hover:text-white flex items-center justify-center transition-all cursor-pointer active:scale-90" title="Tutup">
        ...
        <div class="space-y-2.5">
            ...
            <div class="rounded-2xl bg-white/[0.06] border border-white/15 p-2 sm:p-2.5 flex items-center gap-2.5 sm:gap-3">
                <div class="w-10 h-10 rounded-xl overflow-hidden shrink-0 border border-white/15 bg-black/40">
                    <img src="${trackCover}" class="w-full h-full object-cover" onerror="this.src='/icon.png'" />
                </div>
                <div class="flex-1 min-w-0">
                    <h4 class="font-bold text-xs sm:text-sm text-white truncate">${es(trackTitle)}</h4>
                    <p class="text-[11px] text-white/50 truncate mt-0.5">${es(trackArtist)}</p>
                </div>
                <span class="px-2 py-0.5 rounded-md bg-white/10 text-white/80 text-[9px] font-black uppercase tracking-wider border border-white/10 shrink-0">
                    LAGU DIPILIH
                </span>
            </div>

            <!-- Action Card: Buat Playlist Baru -->
            <div onclick="popup_createPlaylistForSong();" class="rounded-2xl bg-white/[0.06] hover:bg-white/[0.12] border border-white/15 hover:border-white/30 p-2 sm:p-2.5 flex items-center gap-2.5 sm:gap-3 cursor-pointer transition-all active:scale-[0.98] group">
                <div class="w-9 h-9 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                    <i data-lucide="plus" class="w-4 h-4"></i>
                </div>
                <div class="flex-1 min-w-0">
                    <h4 class="font-bold text-xs sm:text-sm text-white truncate">Buat Playlist Baru</h4>
                    <p class="text-[11px] text-white/50 truncate mt-0.5">Simpan ke playlist khusus</p>
                </div>
                <i data-lucide="chevron-right" class="w-4 h-4 text-white/40 group-hover:text-white/80 transition-colors"></i>
            </div>
    ...
    window.popup_addSongToPlaylist = function(plId) {
        addToPlaylistById(plId, track);
        if (popup && popup.parentElement) popup.remove();
        if (typeof Library !== 'undefined' && Library.render) Library.render();
    };

    window.popup_createPlaylistForSong = function() {
        if (popup && popup.parentElement) popup.remove();
        if (typeof Library !== 'undefined' && Library.createNew) {
            Library.createNew(function(newPlId) {
                if (newPlId) {
                    addToPlaylistById(newPlId, track);
                }
            });
        }
    };
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                ...
                '<span class="text-[10px] text-[#6b7280] tracking-wider font-semibold uppercase">MusifyStar Web App</span>' +
            '</div>' +
        '</div>' +
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: 'Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di MusifyStar!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
...
function downloadShareCard() {
    ...
    img.onload = function() {
        ...
        ctx.fillText('DIDENGARKAN DI MusifyStar', 300, 710);
