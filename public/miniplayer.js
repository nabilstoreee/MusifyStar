var MP={
    init(){
        ...
        <div id="mini-player" class="hidden fixed left-4 right-4 sm:left-auto sm:right-6 sm:w-96 z-[160]" style="bottom:78px;transition:transform 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275), bottom 0.25s ease;will-change:transform, bottom;transform:translate3d(0,150px,0);">
            <div id="mini-player-inner" onclick="FullPlayer.open()" class="rounded-full px-2.5 py-2 sm:px-3 sm:py-2.5 flex items-center gap-2.5 sm:gap-3 cursor-pointer active:scale-[0.98] transition-all relative overflow-hidden bg-white/[0.08] backdrop-blur-2xl border border-white/20 hover:border-white/35 shadow-2xl group select-none">
                
                <!-- Overlay for Next Song Transition (AutoNext last 10s) -->
                <div id="mini-next-overlay" class="absolute inset-0 pointer-events-none opacity-0 z-20 flex items-center gap-3 px-3 py-2 rounded-full bg-black/60 border border-white/25 backdrop-blur-2xl overflow-hidden">
                    <div class="relative w-10 h-10 shrink-0 flex items-center justify-center">
                        <div class="w-[32px] h-[32px] rounded-full overflow-hidden border border-white/20 bg-black/40">
                ...
                <div id="mini-beats-bg" class="absolute inset-0 pointer-events-none opacity-30 transition-opacity duration-500 overflow-hidden rounded-full z-0 hidden">
                ...
                <div class="relative w-10 h-10 sm:w-11 sm:h-11 shrink-0 flex items-center justify-center z-10" onclick="FullPlayer.open(); if(typeof event !== 'undefined') event.stopPropagation();">
                    <svg class="w-10 h-10 sm:w-11 sm:h-11 -rotate-90 pointer-events-none absolute inset-0 z-10" viewBox="0 0 48 48">
                        <circle cx="24" cy="24" r="21" fill="none" stroke="rgba(255,255,255,0.15)" stroke-width="2.5"></circle>
                    ...
                    <div class="w-[32px] h-[32px] sm:w-[35px] sm:h-[35px] rounded-full overflow-hidden z-0 border border-white/15 bg-black/40">
                ...
                <div class="flex-1 min-w-0 z-10">
                    ...
                    <div id="mini-artist" class="text-white/60 text-[11px] truncate mt-0.5"></div>
                </div>
                ...
                <div class="flex items-center gap-1 z-10 shrink-0">
                    <button onclick="TP(); if(typeof event !== 'undefined') event.stopPropagation();" class="text-white active:scale-90 p-0.5 cursor-pointer" title="Putar/Jeda">
                        <div id="mini-play-btn" class="w-8 h-8 rounded-full flex items-center justify-center transition-all bg-white/10 border border-white/20 hover:bg-white/20">
                            <i data-lucide="play" class="w-3.5 h-3.5 fill-current ml-0.5"></i>
                        </div>
                    </button>
                    <button id="mini-like-btn" onclick="toggleCurrentLike(); if(typeof event !== 'undefined') event.stopPropagation();" class="text-white/60 hover:text-rose-400 active:scale-90 p-1 cursor-pointer" title="Sukai Lagu">
                        <i data-lucide="heart" class="w-4 h-4 sm:w-4.5 sm:h-4.5"></i>
                    </button>
                    <button id="mini-close-btn" onclick="MP.close(event)" class="text-white/60 hover:text-white active:scale-90 p-1 cursor-pointer transition-colors" title="Tutup Pemutar">
    ...
    applyColors(colors) {
        ...
        if (inner) {
            inner.style.background = "linear-gradient(135deg, color-mix(in srgb, " + tertiary + " 22%, rgba(255, 255, 255, 0.08)), rgba(255, 255, 255, 0.04))";
            inner.style.borderColor = "color-mix(in srgb, " + primary + " 30%, rgba(255, 255, 255, 0.2))";
        }
        ...
        if (playBtn) {
            playBtn.style.borderColor = "color-mix(in srgb, " + primary + " 40%, rgba(255, 255, 255, 0.2))";
        }
