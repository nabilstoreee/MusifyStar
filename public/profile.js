var Profile = {
        ...
        <div class="w-full max-w-4xl h-[90vh] max-h-[850px] bg-[#11131a] border border-white/20 rounded-3xl shadow-2xl overflow-hidden relative flex flex-col" style="box-shadow: 0 25px 60px -15px rgba(244,63,94,0.25);">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span id="admin-header-maintenance-badge" class="hidden text-[10px] bg-rose-500/20 text-rose-400 border border-rose-500/40 font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                                <span class="w-1.5 h-1.5 rounded-full bg-rose-400 animate-pulse"></span> Maintenance Aktif
                            </span>
                        </div>
            ...
            <div class="px-5 py-2.5 bg-black/40 border-b border-white/10 flex items-center justify-between gap-3 text-xs shrink-0 overflow-x-auto hide-scrollbar">
                <div class="flex items-center gap-2">
                    ...
                    <button id="admin-tab-btn-maintenance" onclick="Profile.setAdminTab('maintenance')" class="px-3 py-1.5 rounded-xl font-bold transition-all flex items-center gap-1.5 cursor-pointer bg-white/5 hover:bg-white/10 text-white/70 hover:text-white whitespace-nowrap">
                        <i data-lucide="wrench" class="w-4 h-4 text-amber-400"></i>
                        <span>Mode Pemeliharaan</span>
                        <span id="admin-maintenance-tab-badge" class="hidden text-[9px] bg-rose-500 text-white px-2 py-0.5 rounded-full font-black uppercase tracking-wider animate-pulse shadow-sm shadow-rose-500/50">AKTIF</span>
                    </button>
            ...
            <div class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 hide-scrollbar">
                ...
                <!-- TAB: MAINTENANCE MODE TOGGLE & SETTINGS -->
                <div id="admin-view-maintenance" class="hidden space-y-4">
                    <div id="admin-maintenance-container">
                        <!-- Populated by renderAdminMaintenanceTab -->
                    </div>
                </div>
    ...
    setAdminTab(tab) {
        ...
        var btnMaintenance = gid('admin-tab-btn-maintenance');
        ...
        var viewMaintenance = gid('admin-view-maintenance');
        ...
        if (btnMaintenance) btnMaintenance.className = tab === 'maintenance' ? activeBtnClass : inactiveBtnClass;
        ...
        if (viewMaintenance) viewMaintenance.classList.toggle('hidden', tab !== 'maintenance');
        ...
        if (tab === 'analytics') {
            ...
            Profile.renderAdminMaintenanceTab();
        } else if (tab === 'theme') {
    ...
    refreshAdminData(silent) {
        ...
        if (silent) {
            ...
            if (typeof Maintenance !== 'undefined') {
                Maintenance.checkStatus(function(active) {
                    var tabBadge = gid('admin-maintenance-tab-badge');
                    if (tabBadge) tabBadge.classList.toggle('hidden', !active);
                    var headerBadge = gid('admin-header-maintenance-badge');
        ...
        if (Profile.adminActiveTab === 'maintenance') {
            Profile.renderAdminMaintenanceTab();
                ...
                <div onclick="Profile.selectThemeCard('${t.id}')" class="p-4 rounded-2xl border transition-all cursor-pointer relative group ${isActive ? 'bg-gradient-to-br from-rose-500/15 via-purple-500/10 to-transparent border-rose-500/50 shadow-lg shadow-rose-500/20 ring-1 ring-rose-500/40' : 'bg-white/[0.03] border-white/10 hover:border-white/20 hover:bg-white/[0.05]'}">
                    <div class="flex items-start gap-3.5">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-white/60 line-clamp-2 leading-relaxed">${t.description}</p>
    ...
    async renderAdminMaintenanceTab(silent) {
        var container = gid('admin-maintenance-container');
        if (!container) return;

        // If user is actively typing in inputs, do not overwrite unless forced
        var activeEl = document.activeElement;
        var isTyping = activeEl && (activeEl.id === 'admin-maint-title' || activeEl.id === 'admin-maint-message' || activeEl.id === 'admin-maint-time');
        if (silent && isTyping) {
            return;
        }
        ...
        if (!silent && (!container.innerHTML || container.innerHTML.includes('loader-2'))) {
            ...
            <div class="text-center py-12 text-white/50 space-y-2">
                <i data-lucide="loader-2" class="w-8 h-8 animate-spin mx-auto text-amber-400"></i>
                <p class="text-xs">Memeriksa status mode pemeliharaan...</p>
            </div>`;
            lucide.createIcons();
        }
        ...
        try {
            ...
            var data = await res.json();
            var isEnabled = Boolean(data && (data.maintenance || data.enabled));
            var title = (data && data.title) || 'Pemeliharaan Sistem Musik';
            var message = (data && data.message) || 'Sistem saat ini sedang dalam proses pemeliharaan dan peningkatan server besar. Pemutaran lagu dihentikan sementara agar update berjalan lancar.';
            var estimatedEndTime = (data && data.estimatedEndTime) || '';
            var allowAdminBypass = data && data.allowAdminBypass !== false;
            var updatedAt = (data && data.updatedAt) ? new Date(data.updatedAt).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' }) : 'Belum diatur';

            // Sync global Maintenance object if available
            if (typeof Maintenance !== 'undefined') {
                Maintenance.isActive = isEnabled;
                Maintenance.config = {
                    enabled: isEnabled,
                    title: title,
                    message: message,
                ...
                Maintenance.updateUI(isEnabled);
            }

            var tabBadge = gid('admin-maintenance-tab-badge');
            if (tabBadge) tabBadge.classList.toggle('hidden', !isEnabled);
            var headerBadge = gid('admin-header-maintenance-badge');
            if (headerBadge) headerBadge.classList.toggle('hidden', !isEnabled);

            container.innerHTML = `
            <div class="space-y-4">
                <!-- Status Banner Card -->
                <div class="p-4 sm:p-5 rounded-2xl ${isEnabled ? 'bg-gradient-to-r from-rose-500/20 via-amber-500/15 to-transparent border border-rose-500/40 shadow-lg shadow-rose-500/10' : 'bg-gradient-to-r from-emerald-500/15 via-teal-500/10 to-transparent border border-emerald-500/30'} flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-2xl ${isEnabled ? 'bg-gradient-to-tr from-rose-500 to-amber-500 animate-pulse text-white shadow-lg shadow-rose-500/30' : 'bg-gradient-to-tr from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/20'} flex items-center justify-center shrink-0">
                            <i data-lucide="${isEnabled ? 'wrench' : 'check-circle-2'}" class="w-6 h-6"></i>
                        </div>
                        <div>
                            <div class="flex items-center gap-2">
                                <h3 class="text-base font-black text-white">Mode Pemeliharaan (Maintenance)</h3>
                                <span class="text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider ${isEnabled ? 'bg-rose-500 text-white shadow-md shadow-rose-500/40 animate-pulse' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'}">
                                    ${isEnabled ? '🔴 AKTIF (TERKUNCI)' : '🟢 NORMAL (TERBUKA)'}
                                </span>
                            </div>
                            <p class="text-xs text-white/60 mt-0.5">
                                ${isEnabled 
                                    ? 'Pemutaran musik dijeda di seluruh perangkat pengguna. Layar pemeliharaan aktif saat lagu diklik.' 
                                    : 'Aplikasi beroperasi normal. Seluruh pengguna di semua perangkat dapat mendengarkan musik tanpa kendala.'}
                            </p>
                            <p class="text-[10px] text-white/40 mt-1 font-mono">Pembaruan terakhir: ${updatedAt}</p>
                        </div>
                    </div>

                    <!-- Quick Switch Toggle Button -->
                    <div class="shrink-0 flex items-center gap-2">
                        <button type="button" onclick="Profile.quickToggleMaintenance(${!isEnabled})" class="px-4 py-2 rounded-xl font-black text-xs transition-all flex items-center gap-2 cursor-pointer shadow-lg active:scale-95 ${isEnabled ? 'bg-emerald-500 hover:bg-emerald-400 text-black shadow-emerald-500/25' : 'bg-rose-500 hover:bg-rose-400 text-white shadow-rose-500/25'}">
                            <i data-lucide="${isEnabled ? 'play' : 'power'}" class="w-4 h-4"></i>
                            <span>${isEnabled ? 'Matikan Maintenance (Buka Musik)' : 'Aktifkan Maintenance Sekarang'}</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    ...
                    <div class="lg:col-span-7 space-y-4">
                        <div class="p-4 sm:p-5 rounded-2xl bg-black/40 border border-white/10 space-y-4">
                            <div class="flex items-center justify-between pb-3 border-b border-white/10">
                                <h4 class="text-sm font-bold text-white flex items-center gap-2">
                                    <i data-lucide="sliders" class="w-4 h-4 text-amber-400"></i>
                                    <span>Konfigurasi Layar Pemeliharaan</span>
                                </h4>
                                <span class="text-[10px] text-white/40">Sync Global Real-Time</span>
                            </div>

                            <!-- Enable / Disable Switch -->
                            <div class="p-3 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-between">
                                <div>
                                    <label for="admin-maint-enabled" class="text-xs font-bold text-white block cursor-pointer">Status Mode Pemeliharaan</label>
                                    <p class="text-[11px] text-white/50">Centang untuk memblokir pemutaran musik sementara di semua perangkat</p>
                                </div>
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" id="admin-maint-enabled" class="sr-only peer" ${isEnabled ? 'checked' : ''} onchange="Profile.updateMaintenanceLivePreview()">
                            ...
                            <!-- Judul Layar -->
                            <div>
                                <label class="block text-xs font-bold text-white/80 mb-1.5 flex items-center justify-between">
                                    <span>Judul Layar Pemeliharaan</span>
                                    <span class="text-[10px] text-white/40">Tampil di popup & banner</span>
                                </label>
                                <input type="text" id="admin-maint-title" value="${title.replace(/"/g, '&quot;')}" oninput="Profile.updateMaintenanceLivePreview()" class="w-full px-3.5 py-2.5 rounded-xl bg-black/50 border border-white/10 focus:border-amber-400 focus:outline-none text-xs text-white font-medium transition-all" placeholder="Contoh: Pemeliharaan Server Musik">
                            </div>

                            <!-- Pesan Untuk Pengguna -->
                            <div>
                                <label class="block text-xs font-bold text-white/80 mb-1.5 flex items-center justify-between">
                                    <span>Pesan / Penjelasan Pembaruan</span>
                                    <span class="text-[10px] text-white/40">Jelaskan update besar ke pengguna</span>
                                </label>
                                <textarea id="admin-maint-message" rows="3" oninput="Profile.updateMaintenanceLivePreview()" class="w-full px-3.5 py-2.5 rounded-xl bg-black/50 border border-white/10 focus:border-amber-400 focus:outline-none text-xs text-white font-sans transition-all leading-relaxed" placeholder="Contoh: Sistem sedang dalam proses pemeliharaan dan peningkatan server besar...">${message}</textarea>
                            </div>

                            <!-- Estimasi Waktu Selesai -->
                            <div>
                                <label class="block text-xs font-bold text-white/80 mb-1.5 flex items-center justify-between">
                                    <span>Perkiraan Selesai (Opsional)</span>
                                    <span class="text-[10px] text-amber-400/80">Contoh: 30 Menit, 1 Jam, 21:00 WIB</span>
                                </label>
                                <div class="relative">
                                    <i data-lucide="clock" class="w-4 h-4 text-white/40 absolute left-3.5 top-1/2 -translate-y-1/2"></i>
                                    <input type="text" id="admin-maint-time" value="${estimatedEndTime.replace(/"/g, '&quot;')}" oninput="Profile.updateMaintenanceLivePreview()" class="w-full pl-10 pr-3.5 py-2.5 rounded-xl bg-black/50 border border-white/10 focus:border-amber-400 focus:outline-none text-xs text-white font-mono transition-all" placeholder="Misal: 30 Menit atau Pukul 20:00 WIB">
                                </div>
                            </div>
                            ...
                            <div class="p-3 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-between">
                                <div>
                                    <label for="admin-maint-bypass" class="text-xs font-bold text-white block cursor-pointer">Bypass Pemutaran untuk Admin</label>
                                    <p class="text-[11px] text-white/50">Izinkan admin tetap bisa memutar audio untuk pengujian selama maintenance</p>
                                </div>
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" id="admin-maint-bypass" class="sr-only peer" ${allowAdminBypass ? 'checked' : ''}>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-white/80 mb-2">Template Cepat (Quick Presets):</label>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <button type="button" onclick="Profile.applyMaintenancePreset('big_update')" class="p-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-left text-xs transition-all flex items-start gap-2 active:scale-95 cursor-pointer">
                                        <i data-lucide="zap" class="w-4 h-4 text-amber-400 shrink-0 mt-0.5"></i>
                                        <div>
                                            <p class="font-bold text-white">⚡ Update Server Besar</p>
                                            <p class="text-[10px] text-white/50">Estimasi: 30 Menit</p>
                                        </div>
                                    </button>
                                    <button type="button" onclick="Profile.applyMaintenancePreset('routine')" class="p-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-left text-xs transition-all flex items-start gap-2 active:scale-95 cursor-pointer">
                                        <i data-lucide="wrench" class="w-4 h-4 text-sky-400 shrink-0 mt-0.5"></i>
                                        <div>
                                            <p class="font-bold text-white">🛠️ Pemeliharaan Rutin</p>
                                            <p class="text-[10px] text-white/50">Estimasi: 15 Menit</p>
                                        </div>
                                    </button>
                                    <button type="button" onclick="Profile.applyMaintenancePreset('features')" class="p-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-left text-xs transition-all flex items-start gap-2 active:scale-95 cursor-pointer">
                                        <i data-lucide="sparkles" class="w-4 h-4 text-emerald-400 shrink-0 mt-0.5"></i>
                                        <div>
                                            <p class="font-bold text-white">✨ Rilis Fitur Baru</p>
                                            <p class="text-[10px] text-white/50">Estimasi: 45 Menit</p>
                                        </div>
                                    </button>
                                    <button type="button" onclick="Profile.applyMaintenancePreset('emergency')" class="p-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-left text-xs transition-all flex items-start gap-2 active:scale-95 cursor-pointer">
                                        <i data-lucide="alert-triangle" class="w-4 h-4 text-rose-400 shrink-0 mt-0.5"></i>
                                        <div>
                                            <p class="font-bold text-white">⚠️ Perbaikan Darurat</p>
                                            <p class="text-[10px] text-white/50">Estimasi: Secepatnya</p>
                                        </div>
                                    </button>
                                </div>
                            </div>
                            ...
                            <!-- Save / Apply Button -->
                            <div class="pt-2">
                                <button type="button" id="btn-save-maintenance" onclick="Profile.saveAdminMaintenanceConfig()" class="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 via-rose-500 to-amber-600 hover:opacity-90 active:scale-95 text-white font-bold text-xs shadow-lg shadow-amber-500/25 transition-all flex items-center justify-center gap-2 cursor-pointer">
                                    <i data-lucide="save" class="w-4 h-4"></i>
                                    <span>Simpan & Terapkan Pengaturan</span>
                                </button>
                                <div id="admin-maint-status-box" class="hidden mt-3 p-3 rounded-xl text-xs flex items-center gap-2"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-5 space-y-4">
                        <div class="p-4 sm:p-5 rounded-2xl bg-black/40 border border-white/10 space-y-3">
                            <div class="flex items-center justify-between pb-2 border-b border-white/10">
                                <h4 class="text-xs font-bold text-white flex items-center gap-1.5">
                                    <i data-lucide="eye" class="w-3.5 h-3.5 text-sky-400"></i>
                                    <span>Pratinjau Layar di Perangkat Pengguna</span>
                                </h4>
                                <span class="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/60 font-mono">Live Mockup</span>
                            </div>

                            <p class="text-[11px] text-white/50">Berikut tampilan popup yang akan muncul ketika pengguna mencoba memutar musik saat maintenance aktif:</p>

                            <!-- Mobile Frame Simulator -->
                            <div class="p-4 rounded-2xl bg-[#0e1017] border border-white/15 shadow-inner text-white space-y-3 relative overflow-hidden">
                                <div class="text-center pt-2">
                                    <div class="w-12 h-12 mx-auto mb-2.5 rounded-2xl bg-gradient-to-tr from-amber-500 via-rose-500 to-amber-600 flex items-center justify-center text-white shadow-md shadow-amber-500/30 border border-amber-400/40">
                                        <i data-lucide="wrench" class="w-6 h-6 animate-pulse"></i>
                                    </div>
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-amber-500/20 text-amber-300 border border-amber-500/40 mb-1.5">
                                        <span class="w-1 h-1 rounded-full bg-amber-400 animate-pulse"></span>
                                        Mode Pemeliharaan
                                    </span>
                                    <h5 id="preview-maint-title" class="text-sm font-black text-white tracking-tight">${title}</h5>
                                </div>

                                <div id="preview-maint-message" class="p-3 rounded-xl bg-white/[0.04] border border-white/10 text-[11px] text-white/80 leading-relaxed text-center font-sans">
                                    ${message}
                                </div>

                                <div id="preview-maint-time-box" class="${estimatedEndTime ? '' : 'hidden'} p-2.5 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center gap-2 text-[10px] text-amber-300 font-medium">
                                    <i data-lucide="clock" class="w-3.5 h-3.5 shrink-0"></i>
                                    <span>Estimasi: <strong id="preview-maint-time">${estimatedEndTime}</strong></span>
                                </div>

                                <div class="space-y-1.5 pt-1">
                                    <div class="w-full py-2 rounded-xl bg-gradient-to-r from-amber-500 to-rose-600 text-white font-bold text-[11px] flex items-center justify-center gap-1.5 shadow-md">
                                        <i data-lucide="refresh-cw" class="w-3 h-3"></i>
                                        <span>Cek Status Server</span>
                                    </div>
                                    <div class="w-full py-1.5 rounded-xl bg-white/5 text-white/50 text-[10px] text-center font-semibold">
                                        Tutup Layar
                                    </div>
                                </div>
                            </div>

                            <!-- Card Banner Preview (Under announcement banner) -->
                            <div class="pt-2">
                                <p class="text-[10px] text-white/50 mb-1.5 font-bold uppercase tracking-wider">Tampilan Banner (Di Bawah Banner Pengumuman):</p>
                                <div class="p-3 rounded-xl border border-amber-500/40 bg-gradient-to-r from-amber-500/20 via-black/40 to-transparent flex items-start gap-2.5 text-xs text-white">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center justify-between gap-1">
                                            <span class="text-[9px] font-bold text-amber-400 uppercase tracking-wider">MODE PEMELIHARAAN</span>
                                            <span class="px-2 py-0.5 rounded-lg bg-amber-500/20 text-amber-300 text-[9px] font-bold shrink-0">Detail</span>
                                        </div>
                                        <p class="font-bold text-[11px] truncate text-white mt-0.5">Mode Pemeliharaan Aktif</p>
                                        <p class="text-[10px] text-white/70 truncate">Pemutaran musik dijeda sementara untuk update server</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            container.innerHTML = `
            <div class="p-6 rounded-2xl bg-red-500/10 border border-red-500/30 text-center space-y-2">
                <i data-lucide="alert-circle" class="w-8 h-8 text-red-400 mx-auto"></i>
                <p class="text-xs text-red-300">Gagal memuat status pemeliharaan: ${err.message}</p>
                <button onclick="Profile.renderAdminMaintenanceTab()" class="px-3 py-1.5 rounded-xl bg-red-500 text-white text-xs font-bold">Coba Lagi</button>
            </div>`;
            lucide.createIcons();
        }
    },

    applyMaintenancePreset(presetId) {
        var titleInput = gid('admin-maint-title');
        var msgInput = gid('admin-maint-message');
        var timeInput = gid('admin-maint-time');
        var enabledCheckbox = gid('admin-maint-enabled');

        if (presetId === 'big_update') {
            if (titleInput) titleInput.value = 'Pembaruan Server Besar';
            if (msgInput) msgInput.value = 'Sedang dilakukan migrasi server dan peningkatan sistem besar agar aplikasi berjalan lebih cepat dan stabil. Pemutaran lagu dijeda sementara.';
            if (timeInput) timeInput.value = '30 Menit';
            if (enabledCheckbox) enabledCheckbox.checked = true;
        } else if (presetId === 'routine') {
            if (titleInput) titleInput.value = 'Pemeliharaan Sistem Rutin';
            if (msgInput) msgInput.value = 'Kami sedang melakukan pemeliharaan database dan jaringan musik berkala. Pemutaran akan segera dibuka kembali setelah selesai.';
            if (timeInput) timeInput.value = '15 Menit';
            if (enabledCheckbox) enabledCheckbox.checked = true;
        } else if (presetId === 'features') {
            if (titleInput) titleInput.value = 'Menyiapkan Rilis Fitur Baru';
            if (msgInput) msgInput.value = 'MusifyStar sedang menyiapkan update fitur dan koleksi lagu baru untuk Anda. Harap tunggu sebentar sampai proses instalasi selesai!';
            if (timeInput) timeInput.value = '45 Menit';
            if (enabledCheckbox) enabledCheckbox.checked = true;
        } else if (presetId === 'emergency') {
            if (titleInput) titleInput.value = 'Perbaikan Darurat Server';
            if (msgInput) msgInput.value = 'Terjadi kendala teknis pada upstream server audio. Tim teknis sedang melakukan penanganan darurat secepat mungkin.';
            if (timeInput) timeInput.value = 'Secepatnya';
            if (enabledCheckbox) enabledCheckbox.checked = true;
        }

        Profile.updateMaintenanceLivePreview();
        if (typeof showToast === 'function') {
            showToast('Template preset berhasil diterapkan ke formulir');
        }
    },
    ...
    updateMaintenanceLivePreview() {
        ...
        var prevTitle = gid('preview-maint-title');
        var prevMsg = gid('preview-maint-message');
        var prevTime = gid('preview-maint-time');
        var prevTimeBox = gid('preview-maint-time-box');

        var titleVal = (titleInput && titleInput.value) ? titleInput.value.trim() : 'Pemeliharaan Sistem Musik';
        var msgVal = (msgInput && msgInput.value) ? msgInput.value.trim() : 'Sistem saat ini sedang dalam proses pemeliharaan server besar.';
        var timeVal = (timeInput && timeInput.value) ? timeInput.value.trim() : '';

        if (prevTitle) prevTitle.innerText = titleVal;
        if (prevMsg) prevMsg.innerText = msgVal;
        if (prevTime) prevTime.innerText = timeVal;
        if (prevTimeBox) {
            prevTimeBox.classList.toggle('hidden', !timeVal);
        }
    },

    async quickToggleMaintenance(targetState) {
        var token = sessionStorage.getItem('musifystar_admin_token');
        if (!token) {
            if (typeof showToast === 'function') showToast('Sesi login admin kadaluarsa');
            return;
        }

        try {
            var currentTitle = gid('admin-maint-title') ? gid('admin-maint-title').value : 'Pemeliharaan Sistem Musik';
            var currentMessage = gid('admin-maint-message') ? gid('admin-maint-message').value : 'Sistem sedang dalam proses pemeliharaan dan update besar.';
            var currentTime = gid('admin-maint-time') ? gid('admin-maint-time').value : '';
            var currentBypass = gid('admin-maint-bypass') ? gid('admin-maint-bypass').checked : true;

            var res = await fetch('/api/maintenance', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-admin-token': token
                ...
                body: JSON.stringify({
                    ...
                    allowAdminBypass: currentBypass
                })
            });
            var data = await res.json();
            ...
            if (data.status) {
                if (typeof showToast === 'function') {
                    showToast(targetState ? '🔴 Mode Pemeliharaan DIAKTIFKAN!' : '🟢 Mode Pemeliharaan DINONAKTIFKAN!');
                }
                Profile.renderAdminMaintenanceTab(false);
            } else {
                if (typeof showToast === 'function') {
                    showToast(data.message || 'Gagal mengubah status pemeliharaan');
                }
            }
        } catch (err) {
            if (typeof showToast === 'function') {
    ...
    async saveAdminMaintenanceConfig() {
        var token = sessionStorage.getItem('musifystar_admin_token');
        if (!token) {
            if (typeof showToast === 'function') showToast('Sesi admin tidak valid');
            return;
        }
        ...
        var msgInput = gid('admin-maint-message');
        var timeInput = gid('admin-maint-time');
        var bypassCheckbox = gid('admin-maint-bypass');
        var saveBtn = gid('btn-save-maintenance');
        var statusBox = gid('admin-maint-status-box');

        var enabled = enabledCheckbox ? enabledCheckbox.checked : false;
        var title = titleInput ? titleInput.value.trim() : '';
        var message = msgInput ? msgInput.value.trim() : '';
        var estimatedEndTime = timeInput ? timeInput.value.trim() : '';
        var allowAdminBypass = bypassCheckbox ? bypassCheckbox.checked : true;

        if (!title) {
            if (typeof showToast === 'function') showToast('Judul pemeliharaan tidak boleh kosong');
            return;
        }
        ...
        if (saveBtn) {
            saveBtn.disabled = true;
            saveBtn.classList.add('opacity-70');
            saveBtn.innerHTML = '<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i><span>Menyimpan pengaturan...</span>';
            lucide.createIcons();
        }
        ...
        try {
            ...
            if (data.status) {
                if (statusBox) {
                    statusBox.className = 'mt-3 p-3 rounded-xl text-xs flex items-center gap-2 bg-emerald-500/15 border border-emerald-500/30 text-emerald-300';
                    statusBox.innerHTML = '<i data-lucide="check-circle" class="w-4 h-4 shrink-0"></i><span>' + data.message + '</span>';
                    statusBox.classList.remove('hidden');
                    lucide.createIcons();
                }
                if (typeof showToast === 'function') {
                    showToast(enabled ? 'Mode Pemeliharaan Aktif' : 'Mode Pemeliharaan Dinonaktifkan');
                }
                Profile.renderAdminMaintenanceTab(true);
            } else {
                if (statusBox) {
                    statusBox.className = 'mt-3 p-3 rounded-xl text-xs flex items-center gap-2 bg-red-500/15 border border-red-500/30 text-red-300';
                    statusBox.innerHTML = '<i data-lucide="alert-circle" class="w-4 h-4 shrink-0"></i><span>' + (data.message || 'Gagal menyimpan') + '</span>';
                    statusBox.classList.remove('hidden');
                    lucide.createIcons();
                }
            }
            ...
            if (statusBox) {
                statusBox.className = 'mt-3 p-3 rounded-xl text-xs flex items-center gap-2 bg-red-500/15 border border-red-500/30 text-red-300';
                statusBox.innerHTML = '<i data-lucide="wifi-off" class="w-4 h-4 shrink-0"></i><span>Koneksi bermasalah saat menyimpan data.</span>';
                statusBox.classList.remove('hidden');
                lucide.createIcons();
            }
        } finally {
            if (saveBtn) {
                saveBtn.disabled = false;
                saveBtn.classList.remove('opacity-70');
                saveBtn.innerHTML = '<i data-lucide="save" class="w-4 h-4"></i><span>Simpan & Terapkan Pengaturan</span>';
                lucide.createIcons();
            }
        }
    }
